...
if [ -f "tester-quantum-ui.sh" ] && [ -f "demo-quantum-final.sh" ] && [ -f "⚙️ scripts/test-creatures-quantiques.sh" ]; then
    echo -e "${GREEN}✅ OK${NC}"
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
    echo -e "${RED}❌ ÉCHEC${NC}"
...
if [ -f "🖥️ backend/src/main/resources/quantum-creatures.json" ] && [ -f "⚙️ scripts/test-creatures-quantiques.sh" ]; then
    echo -e "${GREEN}✅ OK${NC}"
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
    echo -e "${RED}❌ ÉCHEC${NC}"
...
if [ -f "🖥️ backend/src/main/java/com/heroesoftimepoc/temporalengine/service/ScriptTranslationService.java" ] && [ -f "⚙️ scripts/test-service-traduction.sh" ]; then
    echo -e "${GREEN}✅ OK${NC}"
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
    echo -e "${RED}❌ ÉCHEC${NC}"
