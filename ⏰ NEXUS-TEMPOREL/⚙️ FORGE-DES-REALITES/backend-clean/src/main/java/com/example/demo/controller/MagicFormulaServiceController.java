...
public class MagicFormulaServiceController {
    ...
    public ResponseEntity<Map<String, Object>> executeFormula(@RequestBody Map<String, Object> request) {
        ...
        try {
            ...
            // WALTER HTTP FIX: Retourner le bon code HTTP selon le succès
            if (!result.isSuccess()) {
                // Analyser le type d'erreur
                String message = result.getMessage();
                if (message.contains("Formule inconnue") || message.contains("non trouvée")) {
                    return ResponseEntity.status(404).body(response); // Not Found
                } else if (message.contains("invalide") || message.contains("mal formée")) {
