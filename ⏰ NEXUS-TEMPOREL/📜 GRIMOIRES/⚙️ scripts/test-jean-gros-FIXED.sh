...
for script in "test-causality-wall.sh" "test-vision-temporelle.sh" "test-quantum-maze.sh" "test-creatures-quantiques.sh" "test-service-traduction.sh"; do
    if [ -f "$ROOT_DIR/⚙️ scripts/$script" ]; then
        run_test "$(basename "$script" .sh)" "cd '$ROOT_DIR' && timeout 60s ./⚙️ scripts/$script"
    else
...
if [ -f "$ROOT_DIR/⚙️ scripts/actifs/test-ui-quick.sh" ]; then
    run_test "ui-quick" "cd '$ROOT_DIR' && timeout 30s ./⚙️ scripts/actifs/test-ui-quick.sh"
else
...
# Analyse des ressources
{
    ...
    if [ -d "$ROOT_DIR/🖥️ backend/src/main/resources/" ]; then
        find "$ROOT_DIR/🖥️ backend/src/main/resources/" -name "*.json" -exec grep -l "formula" {} \; | head -5
    else
        echo "Répertoire 🖥️ backend/resources non trouvé"
    fi
    echo "\`\`\`"
    echo ""
} >> "$REPORT_FILE"
